...
target = '027600.txt'
extra = 'urqmd/{}'.format(target.split('.')[0] + '/')

with open(target, 'r') as rfile:

	lines = rfile.readlines()

wfile = open('catatool_{}.out'.format(target), 'w')
...
for nth in range(len(patterns)):
	...
	if '?NUMBER?' not in patterns[nth]:
		...
		if len(num_sets[0]) < 5:
			...
			text = '\n'.join([extra + pattern_split[0] + str(i) for i in k])
			...
			text = pattern_split[0] + str(k[0]) + '\n' + extra + pattern_split[0] + str(k[1]) + '\n.\n. {} instances\n.\n'.format(len(num_sets[0])) + extra + pattern_split[0] + str(k[-2]) + '\n' + extra + pattern_split[0] + str(k[-1])
		...
		if num_count > 1:
			...
			for i in num_sets[0]:
				...
				pattern_1 = extra + pattern_split[0] + str(i) + pattern_split[1]
